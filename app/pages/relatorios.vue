<template>
  <main class="flex min-w-0 flex-1 flex-col px-4 py-8 sm:px-6 sm:py-10">
    ...
    <p class="mb-6 font-sans text-sm text-wine-600">
      Visão financeira de compras e despesas. Não representa lucro, margem ou CMV.
    </p>
...
import { completarMeses, resolverPeriodo } from '~/utils/relatorioFinanceiro'
import type { PresetPeriodo, RelatorioFinanceiro } from '~/types/relatorio-financeiro'
...
const relatorio = ref<RelatorioFinanceiro | null>(null)
const carregando = ref(false)
const erro = ref<string | null>(null)

const semDados = computed(() => (relatorio.value?.resumo.quantidade_compras ?? 0) === 0)
