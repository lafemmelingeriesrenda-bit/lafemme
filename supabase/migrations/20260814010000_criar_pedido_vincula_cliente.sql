...
-- Executa em UMA transação (validação + cliente + INSERT pedidos + itens_pedido)
-- SECURITY DEFINER => roda como owner (postgres), ignora RLS.
-- Sem baixar estoque; bloqueia linhas com FOR UPDATE.
--
-- Vínculo de cliente: normaliza o telefone, reutiliza cliente existente
-- ou cria um novo (INSERT ... ON CONFLICT -> race-safe), e grava
-- pedidos.cliente_id. pedidos.nome_cliente/telefone_cliente seguem sendo
-- snapshots do pedido.
-- ============================================================
...
begin
  ...
  -- 5) Normaliza o telefone e reutiliza/cria o cliente (race-safe)
  v_telefone_norm := nullif(regexp_replace(coalesce(p_telefone, ''), '\D', '', 'g'), '');

  if v_telefone_norm is not null then
    insert into public.clientes (nome, telefone, telefone_normalizado)
    values (p_nome, v_telefone_norm, v_telefone_norm)
  ...
  -- 6) Cria o pedido (uma transação só) vinculado ao cliente
  insert into public.pedidos (
    cliente_id, status, subtotal, frete, total,
    nome_cliente, telefone_cliente, observacoes
  )
  values (
    v_cliente_id, 'aguardando_atendimento', v_subtotal, v_frete, v_total,
    p_nome, p_telefone, p_observacoes
  )
  returning id into v_pedido_id;

  -- 7) Cria os itens (snapshot dos dados atuais)
  for v_item in select value from jsonb_array_elements(p_itens) loop
    v_variante_id := (v_item->>'varianteId')::bigint;
    v_quantidade := (v_item->>'quantidade')::integer;
    select * into v_registros from _var_tmp where id = v_variante_id;
